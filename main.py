...
class MainWindow(QtWidgets.QWidget):
    def __init__(self, *args, **kwargs):
        ...
        image_select_hbox = QtWidgets.QHBoxLayout()
        image_select_hbox.addWidget(QtWidgets.QLabel('Selected Image:'))
        image_select_hbox.addWidget(self.filename_edit)
        image_select_hbox.addWidget(image_browse)
        self.restore_image_btn = QtWidgets.QPushButton('Restore')
        ...
        # kernel
        self.kernel_dropdown = QtWidgets.QComboBox()
        ...
        kernel_select_vbox = QtWidgets.QVBoxLayout()
        kernel_select_vbox.addWidget(QtWidgets.QLabel('Select Kernel:'))
        ksel_hbox = QtWidgets.QHBoxLayout()
        ksel_hbox.addWidget(self.kernel_dropdown)
        reset_kernel_btn = QtWidgets.QPushButton()
        reset_kernel_btn.setIcon(QtGui.QIcon('undo-arrow.png'))
        ...
        self.kernel = np.array(Kernels.Identity.value)
        self.kernel_label = QtWidgets.QLabel()
        self.kernel_label.setFont(QtGui.QFont('Courier New', 11))
        self.kernel_label.setText(str(self.kernel))
        kernel_select_vbox.addWidget(self.kernel_label)
        self.persist_kernel = QtWidgets.QCheckBox('Changes persist?', self)
        self.persist_kernel.setChecked(True)
        kernel_select_vbox.addWidget(self.persist_kernel)
        self.apply_kernel_btn = QtWidgets.QPushButton('Apply')
        ...
        kernel_group_box = QtWidgets.QGroupBox('Filtering')
        kernel_group_box.setLayout(kernel_select_vbox)
        
        # thresholding
        self.thresh_dropdown = QtWidgets.QComboBox()
        ...
        thresh_vbox = QtWidgets.QVBoxLayout()
        thresh_vbox.addWidget(self.thresh_dropdown)
        global_thresh_hbox = QtWidgets.QHBoxLayout()
        global_thresh_hbox.addWidget(QtWidgets.QLabel('Global Threshold:'))
        self.global_thresh_spin = QtWidgets.QSpinBox()
        ...
        self.persist_thresh = QtWidgets.QCheckBox('Changes persist?', self)
        self.persist_thresh.setChecked(False)
        thresh_vbox.addWidget(self.persist_thresh)
        self.apply_thresh_btn = QtWidgets.QPushButton('Apply')
        ...
        thresh_group_box = QtWidgets.QGroupBox('Thresholding')
        ...
        self.save_hist_btn = QtWidgets.QPushButton('Save Histogram')
        ...
        hist_group_box = QtWidgets.QGroupBox('Histogram')
        ...
        op_vbox.addWidget(QtWidgets.QLabel('Made using PyQt5, OpenCV, and ♥\n[2024] Priyansh Agrahari'))
    ...
    def set_frame_image(self, image, isBGR = True):
        ...
        if isBGR:
            ...
            qimage = QtGui.QImage(image.data, image.shape[1], image.shape[0], QtGui.QImage.Format_Grayscale8)
    ...
    def set_hist(self, image, isBGR = True):
        ...
        fig.tight_layout()
        fig.canvas.draw()
        himg = np.array(fig.canvas.renderer.buffer_rgba())
        himg = cv2.cvtColor(himg, cv2.COLOR_RGBA2RGB)
        self.currentHistImage = himg
        qimage = QtGui.QImage(himg.data, himg.shape[1], himg.shape[0], QtGui.QImage.Format_RGB888)            
        pixmap = QtGui.QPixmap.fromImage(qimage)
    ...
    def kernel_dropdown_changed(self, s : str):
        if s == "Custom":
            ...
            self.kernel = np.array(Kernels[s].value)
        self.kernel_label.setText(str(self.kernel))
    
    def show_custom_kernel_dialog(self):
        s, ok = QtWidgets.QInputDialog().getMultiLineText(self, "Enter Custom Kernel", 
